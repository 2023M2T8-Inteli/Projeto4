...
async function getMaxMinValue() {
    const [maxFRow1, maxFRow2, minFRow1, minFRow2, maxPRow1, maxPRow2, minPRow1, minPRow2, maxARow1, maxARow2, minARow1, minARow2] = await Promise.all([
        ...
        new Promise((resolve, reject) => {
            db.all(`SELECT MAX(f_maxima) as maxf2 FROM Choque1 WHERE id_viagem=1`, function (err, row) {
                if (err) {
                    reject(err);
                } else {
        ...
        new Promise((resolve, reject) => {
            db.all(`SELECT MIN(f_maxima) as minf2 FROM Choque1 WHERE id_viagem=1`, function (err, row) {
                if (err) {
                    reject(err);
                } else {
        ...
        new Promise((resolve, reject) => {
            db.all(`SELECT MAX(peg) as maxp2 FROM Choque1 WHERE id_viagem=1`, function (err, row) {
                if (err) {
                    reject(err);
                } else {
        ...
        new Promise((resolve, reject) => {
            db.all(`SELECT MIN(peg) as minp2 FROM Choque1 WHERE id_viagem=1`, function (err, row) {
                if (err) {
                    reject(err);
                } else {
        ...
        new Promise((resolve, reject) => {
            db.all(`SELECT MAX(act) as maxa2 FROM Choque1 WHERE id_viagem=1`, function (err, row) {
                if (err) {
                    reject(err);
                } else {
        ...
        new Promise((resolve, reject) => {
            db.all(`SELECT MIN(act) as mina2 FROM Choque1 WHERE id_viagem=1`, function (err, row) {
                if (err) {
                    reject(err);
                } else {
